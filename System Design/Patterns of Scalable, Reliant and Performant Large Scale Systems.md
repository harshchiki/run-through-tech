## Content
- [Lesons for developing Large Scale Systems](#lesons-for-developing-large-scale-systems)
- [Availabitlity Metrics](#availabitlity-metrics)
- [MTBF, MTBR, MTTR, MTTF, MTBDE and FIT](#mtbf-mtbr-mttr-mttf-mtbde-and-fit)
...
# Lesons for developing Large Scale Systems
* Decide on your availability metrics:
    * Everyone should agree on the goals and how to measure them daily. 
    * Remember that harvest and yield are more useful than just uptime.
* Focus on MTTR (Mean time to repair) at least as much as MTBF(Mean time before failure).
    * Repair time is easier to affect for an evolving system and has just as much impact
* Graceful degradation is a critical part of a high-availability strategy
* Automate upgrades as much as possible
* The basic model that giant-scale services follow provides some fundamental advantages: 
    * Access anywhere, anytime
    * Availability via multiple devices. 
    * Groupware support. Centralizing data from many users allows service providers to offer group-based applications such as calendars, teleconferencing systems, and group-management systems 
    * Lower overall cost
    * Simplified service updates
* Reference: https://people.eecs.berkeley.edu/~brewer/papers/GiantScale-IEEE.pdf

# Availabitlity Metrics
* References:
  * https://people.eecs.berkeley.edu/~brewer/papers/GiantScale-IEEE.pdf
* Uptime: 
    * traditional metric
    *  fraction of time a site is handling traffic
    *  typically measured in nines
    *  We can think of uptime as
          *  ![](_resources/2019-12-08-12-38-32.png)
          *  MTBF - Mean time before failure
          *  MTTR - Mean time to repair
    *  Uptime can be improved by
          *  reducing the frequency of failures <-- Looks more pleasing
          *  reducing the time to fix them <-- Easier to accomplish
*  yield:
   *  ![](_resources/2019-12-08-12-40-35.png)
   *  typically close to uptime, but directly maps to user experience.
      *  because it correctly reflects not all seconds have equal value.
      *  being down for one second, has no impact on users, as there were no queries
      *  Similarly, being down for one second at peak and off-peak times generates the same uptime, but vastly different yields because there might be an order-of-magnitude difference in load between the peak second and the minimum-load second
   *  because of these reasons, prefer `yield` over `uptime`

# MTBF, MTBR, MTTR, MTTF, MTBDE and FIT
![](_resources/2019-12-08-12-00-24.png)
* ## Mean Time Between Failure (MTBF):
    * **Reliability**
    * Number of failures per unit time.
* ## Mean Time To Repair (MTTR):
    * Generally in context to hardware modules.
* ## Mean Time Between Replacements (MTBR):
    * Hardware
* ## Mean Time To Failure (MTTF) 
    * **Reliability**
    * Hardware
    * time to first failure
* ## Mean Time Between Downing Event (MTBDE):
* ## Failure In Time (FIT) 
* Reference: https://www.linkedin.com/pulse/mtbf-mtbr-mttr-mttf-mtbde-fit-ivan-luizio-magalh%C3%A3es/
